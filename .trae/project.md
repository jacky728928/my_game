...
| `js/game_core.js` | 游戏核心（主循环、状态管理、敌人生成避开墙体、游戏重启、页面切换自动暂停） | 游戏循环 |
| `js/home.js` | 游戏主页（底部导航栏 + 立绘展示页 + 科技树/商店占位页，页面切换自动控制游戏元素显隐） | 主页导航、页面切换逻辑 |
| `pic/` | 立绘图片资源（主页立绘展示用） | 立绘图片 |
...
## 当前状态
v0.5: 基础骨架 + 6级敌人 + 手枪 + 经验等级系统 + 经验球飞行动画 + 经验吸取范围可视化 + 升级三选一能力系统 + 暴击 + 浮动伤害数字 + **副武器系统**（榴弹炮/手雷/医疗包/**天基屠龙炮**）+ **副武器专属技能词条** + 开发者面板武器控制 + **主动技能系统**（闪烁突袭/冲刺翻滚/疾跑）+ **墙体系统**（矮/中/高三种墙体 + 碰撞/阻挡规则）+ 代码模块化拆分 + **天基屠龙炮蓄力瞄准+炫酷特效+UI警告提示** + **PC端方向键/WASD操控** + **地图编辑器 + 地图选择 + 地图预览** + **test_project 合并（底部导航栏 + 立绘页 + 页面切换自动暂停游戏）**
